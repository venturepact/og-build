...
window.createCookie = function (a, b, c) {
	...
	} else document.cookie = a + "=" + b + d + "; path=/";
...
function initIframe(id) {
	...
	if (og_e.getAttribute('data-bgcolor')) var og_bc = og_e.getAttribute('data-bgcolor');else var og_bc = '#fb545b';
	if (og_e.getAttribute('data-textcolor')) var og_tc = og_e.getAttribute('data-textcolor');else {
		if (og_a == 'outgrow-b') var og_tc = '#ffffff';else var og_tc = '#fb545b';
	}
	if (og_p == 'outgrow-p') {
		if (iWidth > 786) {
	...
	if (og_e.getAttribute('data-embedBorderRadius')) var og_br = og_e.getAttribute('data-embedBorderRadius');else var og_br = '0';
	if (og_e.getAttribute('data-embedFontSize')) var og_fs = og_e.getAttribute('data-embedFontSize');else var og_fs = '12';
	if (og_e.getAttribute('data-isLDrawer')) var og_ld = og_e.getAttribute('data-isLDrawer');else var og_ld = 'false';
	...
	if (og_p == 'outgrow-p') {
		if (og_a == 'outgrow-b') anchorTag.setAttribute("class", "outgrow-b");else if (og_a == 'outgrow-l') anchorTag.setAttribute("class", "outgrow-l");
	...
	if (og_es === 'true' && og_et === 'true') {
		...
		if (og_p == 'outgrow-p') {
			setTimeout(function () {
				toggleSchedulePopup(iframe_id);
			}, embedTimeout);
		}
		if (og_p == 'outgrow-d') {
			setTimeout(function () {
				toggleScheduleDrawer(iframe_id);
			}, embedTimeout);
		}
	}
}
