...
self.settings.mode=self.settings.mode||(self.settings.maxItems===1?'single':'multi');self.settings.dropdownDirection=self.settings.dropdownDirection||'auto';if(typeof self.settings.hideSelected!=='boolean'){self.settings.hideSelected=self.settings.mode==='multi';}
...
$dropdown_content.html(html);if(self.settings.highlight&&results.query.length&&results.tokens.length){for(i=0,n=results.tokens.length;i<n;i++){highlight($dropdown_content,results.tokens[i].regex);}}
...
self.setActiveOption($active);if(triggerDropdown&&!self.isOpen){self.open();}}else{self.setActiveOption(null);if(triggerDropdown&&self.isOpen){self.close();}}
self.positionDropdown();},addOption:function(data){var i,n,value,self=this;if($.isArray(data)){for(i=0,n=data.length;i<n;i++){self.addOption(data[i]);}
return;}
...
self.isOpen=false;self.$dropdown.hide();self.setActiveOption(null);self.refreshState();if(trigger)self.trigger('dropdown_close',self.$dropdown);},positionDropdown:function(){var self=this;var direction=self.settings.dropdownDirection;if(direction!=='auto'){return self.setDropdownDirection(direction);}
self.setDropdownDirection('down');var rect=self.$dropdown[0].getBoundingClientRect();var inViewPort=rect.bottom<=(window.innerHeight||$(window).height());if(!inViewPort){return self.setDropdownDirection('up');}},setDropdownDirection:function(direction){var $control=this.$control;var offset=this.settings.dropdownParent==='body'?$control.offset():$control.position();offset.top+=$control.outerHeight(true);this.$dropdown.css({width:$control.outerWidth(),left:offset.left,top:(direction==='down')?offset.top:'auto',bottom:(direction==='down')?'auto':offset.top,}).toggleClass('dropdown-up',(direction==='down')?false:true);},clear:function(silent){var self=this;if(!self.items.length)return;self.$control.children(':not(input)').remove();self.items=[];self.lastQuery=null;self.setCaret(0);self.setActiveItem(null);self.updatePlaceholder();self.updateOriginalInput({silent:silent});self.refreshState();self.showInput();self.trigger('clear');},insertAtCaret:function($el){var caret=Math.min(this.caretPos,this.items.length);if(caret===0){this.$control.prepend($el);}else{$(this.$control[0].childNodes[caret]).before($el);}
